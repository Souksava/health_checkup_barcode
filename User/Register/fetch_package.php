 ...
 if(mysqli_num_rows($resultPackage_limit) > 0)
{
 $output .= '
  ...
  <table class="table-bordered" style="width: 700px;text-align: center;">
...
$no2_ += 1;
  $output .= '
    <tr>
        <td>
            <a href="#" data-toggle="modal" data-target="#exampleModalAdd_list"
                class="fas fa-plus-circle toolcolor btnAdd_list"></a>&nbsp;&nbsp;
            <a href="#" data-toggle="modal" data-target="#exampleModalAdd_more"
                class="fas fa-user-plus toolcolor btnAdd_more"></a>&nbsp;&nbsp;
...
$('.btnDelete_Package').on('click', function() {
   ...
   $('.btnAdd_list').on('click', function() {
    $('#exampleModalAdd_list').modal('show');
      $tr = $(this).closest('tr');
      var data = $tr.children("td").map(function() {
         return $(this).text();
      }).get();
      ...
      $('#package_add_list').val(data[2]);
      $('#package_add_list_des').val(data[3]);
   });
   $('.btnAdd_more').on('click', function() {
    $('#exampleModalAdd_more').modal('show');
      $tr = $(this).closest('tr');
      var data = $tr.children("td").map(function() {
         return $(this).text();
      }).get();
      ...
      $('#package_add_more').val(data[2]);
      $('#package_add_more_des').val(data[3]);
   });
</script>